...
- **Reading an index for a workspace other than the current one is the case the rule above does not settle.** `--index-*` resolves against the process's own `MMDAPP` and `MDSC_CACHED`, so a per-workspace scan needs its own context established rather than a bare in-process call. That is a design question for the consumer to answer, never a licence to spawn a tool by path.
