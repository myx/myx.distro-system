...
ListDistroDeclares(){
	...
	while [ $# -gt 0 ]; do
		...
		case "$1" in
			...
			--all-filter-and-cut)
				...
				# DistroSystemContext --filter-index-declares "${filter}" cat
			...
			--filter-and-cut)
				...
				local filter="$2"; shift 2
				# DistroSystemContext --intersect-filter-index-declares MDSC_SELECT_PROJECTS "${filter}" cat
			...
			--unroll-filter-and-cut)
				...
				local filter="$2"; shift 2
				DistroSystemContext --intersect-unroll-filter-index-declares MDSC_SELECT_PROJECTS "${filter%':'}:" "$@"
				return 0
			;;
			--unroll-filter-and-cut-merged)
				...
				local filter="$2"; shift 2
				DistroSystemContext --intersect-unroll-filter-index-declares-merged MDSC_SELECT_PROJECTS "${filter%':'}:" "$@"
				return 0
			;;
			--merge-sequence)
				if [ -n "$2" ] ; then
				...
				DistroSystemContext --intersect-index-declares-merged MDSC_SELECT_PROJECTS awk '
					!x[$3]++ { print $0; }
				'
				return 0
			;;
	...
	DistroSystemContext --intersect-index-declares MDSC_SELECT_PROJECTS cat
	return 0
}
	...
	*/sh-scripts/ListDistroDeclares.fn.sh)
		if [ -z "$1" ] || [ "$1" = "--help" ] ; then
			...
			if [ "$1" = "--help" ] ; then
				...
				. "$MDLT_ORIGIN/myx/myx.distro-system/sh-lib/help/Help.ListDistroDeclares.include"
			fi
			exit 1
		fi
