...
AllActions(){
	...
	local sedEx="sed -e 's:^$MMDAPP/source/::' -e 's:^$MDSC_SOURCE/::' -e 's:^$MDLT_ORIGIN/::' -e 's:/actions/: :'"

	. "$MDLT_ORIGIN/myx/myx.distro-system/sh-lib/SystemContext.UseStandardOptions.include"
	while [ $# -gt 0 ]; do
		case "$1" in
			--completion)
				sedEx="sed -e 's:^$MMDAPP/source/::' -e 's:^$MDSC_SOURCE/::' -e 's:^$MDLT_ORIGIN/::' -e 's:^.*/actions/::'"
				shift ; break
			;;
			--executables)
				sedEx="cat"
			...
			;;
			--full)
				sedEx="sed -e 's:^$MMDAPP/source/::' -e 's:^$MDSC_SOURCE/::' -e 's:^$MDLT_ORIGIN/::' -e 's:^\(.*\)/actions/:\1 \1/actions/:'"
				shift ; break
			;;
			--|--default|--all-actions)
				# sedEx="sed -e 's:^$MMDAPP/source/::' -e 's:^$MDSC_SOURCE/::' -e 's:^$MDLT_ORIGIN/::' -e 's:/actions/: :'"
	...
	. "$MDLT_ORIGIN/myx/myx.distro-source/sh-lib/source-context/ScanSourceActions.include" | eval "$sedEx"
}
